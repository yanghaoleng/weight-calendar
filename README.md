...
- 保存后日历格、涨跌数字和方向箭头会给出轻量动效；按钮、输入、导航、保存与错误状态使用 UI SFX 的 Zen 音效反馈。
- 设置页统一管理五种背景颜色、三种西文字体、操作音效、AI 体重分析、Markdown 数据导出、退出登录和注销账户；中文始终使用系统黑体。
- AI 体重分析通过滑块填写身高和估算体脂率，结合近期体重记录，由豆包 Seed 2.0 Mini 按“饮食、运动、睡眠”固定结构生成简洁建议。
