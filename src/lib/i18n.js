...
const messages = {
  "zh-CN": {
    ...
    data: "数据",
    exportData: "导出数据",
    exportHint: "下载含 Markdown、PDF、HTML 的导出包（{count} 条记录）",
    support: "支持",
    donateAuthor: "打赏作者",
    ...
    dayCleared: "当天记录已清空",
    saved: "已保存",
    exportDone: "导出包已下载", exportFailed: "导出失败，请重试",
...
messages["zh-HK"] = {
  ...
  data: "資料", exportData: "匯出資料", exportHint: "下載含 Markdown、PDF、HTML 的匯出包（{count} 條記錄）", support: "支持", account: "帳戶", logout: "登出", deleteAccount: "註銷帳戶",
...
Object.assign(messages["zh-CN"], {
  ...
  cloudSyncLocalLead: "记录会保存在这台设备，并以本地用户编号备份到服务端；开启云端同步后才能跨设备打开。",
  ...
  storedTechnical: "必要技术与行为数据：未开启云同步时，也会使用随机本地用户编号上报当前设置、体重记录、访问路径、页面与功能曝光及点击行为，用于运营后台、使用路径和 CTR 统计。数字键盘只记录“点击了数字键”，不记录密码或体重的具体数字。",
...
Object.assign(messages["zh-HK"], {
  ...
  cloudSyncLocalLead: "記錄會保存在這台裝置，並以本地用戶編號備份到服務端；開啟雲端同步後才能跨裝置打開。",
  ...
  storedTechnical: "必要技術與行為資料：未開啟雲端同步時，也會使用隨機本地用戶編號上傳當前設定、體重記錄、造訪路徑、頁面與功能曝光及點擊行為，用於營運後台、使用路徑和 CTR 統計。數字鍵盤只記錄點擊了數字鍵，不記錄密碼或體重的具體數字。",
...
Object.assign(messages["zh-TW"], {
  ...
  cloudSyncLocalLead: "記錄會儲存在這台裝置，並以本機使用者編號備份到服務端；開啟雲端同步後才能跨裝置開啟。",
  cloudSyncRisk: "瀏覽器快取可能被系統或清理工具刪除。",
  cloudSyncNeverLost: "開啟雲端同步後，資料較不容易遺失。",
  ...
  storedTechnical: "必要技術與行為資料：未開啟雲端同步時，也會使用隨機本機使用者編號上傳當前設定、體重記錄、瀏覽路徑、頁面與功能曝光及點擊行為，用於營運後台、使用路徑和 CTR 統計。數字鍵盤只記錄點擊數字鍵，不記錄密碼或體重的具體數字。",
...
Object.assign(messages.en, {
  ...
  cloudSyncLocalLead: "Records stay on this device and are backed up to the service under a local user ID; cloud sync is still required to open them on another device.",
  ...
  storedTechnical: "Necessary technical and behavioral data: even before cloud sync is enabled, a random local user ID sends current settings, weight records, visit paths, page and feature impressions, and clicks for the operations dashboard, user journeys, and CTR reporting. Numeric keypads record only that a number key was clicked, never the actual passcode or weight digit.",
...
Object.assign(messages.ja, {
  ...
  cloudSyncLocalLead: "記録はこの端末に保存され、ローカルユーザーIDでサーバーにもバックアップされます。別の端末で開くにはクラウド同期が必要です。",
  ...
  storedTechnical: "必要な技術・行動データ：クラウド同期をオンにする前も、ランダムなローカルユーザーIDで現在の設定、体重記録、訪問経路、ページ・機能の表示とクリックを送信し、運用画面、利用経路、CTR集計に使用します。数字キーパッドはキーが押された事実だけを記録し、パスコードや体重の数字は記録しません。",
...
Object.assign(messages.ko, {
  ...
  cloudSyncLocalLead: "기록은 이 기기에 저장되고 로컬 사용자 ID로 서비스에도 백업됩니다. 다른 기기에서 열려면 클라우드 동기화가 필요합니다.",
  ...
  storedTechnical: "필요한 기술 및 행동 데이터: 클라우드 동기화를 켜기 전에도 무작위 로컬 사용자 ID로 현재 설정, 체중 기록, 방문 경로, 페이지·기능 노출 및 클릭을 전송하여 운영 백오피스, 사용 경로, CTR 통계에 사용합니다. 숫자 키패드는 숫자 키를 눌렀다는 사실만 기록하며 암호나 체중 숫자는 기록하지 않습니다.",
...
Object.assign(messages["zh-CN"], {
  ...
  productPositionText: "虽然每天的体重会有波动，短期内的体重变化也不会太大，你可能会怀疑这样的记录是否有价值。\n即使如此，我依然希望它帮你把记录体重这件事，变成一种有仪式感的小习惯。持续记录可以时刻提醒自己关注健康状况，从而在作息、饮食和运动中自主做出调节。",
  syncTipTitle: "建议开启云端同步",
  syncTipText: "当前记录已按本地用户编号备份，但清理浏览器数据后仍可能无法打开。开启同步后，才能用密码跨设备找回。",
  goEnableSync: "去开启",
  doNotRemind: "不再提醒",
  homeTipTitle: "把体重日历放到桌面",
  homeTipText: "添加到主屏幕后，可以像 APP 一样更快打开，还能先选一个喜欢的图标。",
  seeHomeSetup: "查看设置方法",
  personalAppearance: "APP 图标",
...
Object.assign(messages["zh-HK"], {
  ...
  productPositionText: "雖然每天的體重會有波動，短期內的體重變化也不會太大，你可能會懷疑這樣的記錄是否有價值。\n即使如此，我依然希望它幫你把記錄體重這件事，變成一個有儀式感的小習慣。持續記錄可以時刻提醒自己關注健康狀況，從而在作息、飲食和運動中自主作出調節。",
  syncTipTitle: "建議開啟雲端同步", syncTipText: "目前記錄已按本地用戶編號備份，但清理瀏覽器資料後仍可能無法打開。開啟同步後才能用密碼跨裝置找回。", goEnableSync: "去開啟", doNotRemind: "不再提醒",
  homeTipTitle: "把體重日曆放到主畫面", homeTipText: "加入主畫面後可像 APP 一樣更快開啟，也可以先選一個喜歡的圖示。", seeHomeSetup: "查看設定方法",
  personalAppearance: "APP 圖示", customAppIcon: "自訂 APP 圖示", customAppIconHint: "圖示只保存在這部裝置，不會上傳到伺服器。",
...
Object.assign(messages["zh-TW"], {
  anonymousCalendar: "某個人的體重日曆",
  productPositionTitle: "讓記錄成為一個小習慣",
  productPositionText: "雖然每天的體重會波動，短期內的體重變化也不會太大，你可能會懷疑這樣的記錄是否有價值。\n即使如此，我依然希望它幫你把記錄體重這件事，變成一個有儀式感的小習慣。持續記錄可以時刻提醒自己關注健康狀況，從而在作息、飲食和運動中自主做出調整。",
  syncTipTitle: "建議開啟雲端同步", syncTipText: "目前記錄已按本機使用者編號備份，但清理瀏覽器資料後仍可能無法開啟。開啟同步後才能用密碼跨裝置找回。", goEnableSync: "去開啟", doNotRemind: "不再提醒",
  homeTipTitle: "把體重日曆放到主畫面", homeTipText: "加入主畫面後可像 APP 一樣更快開啟，也能先選一個喜歡的圖示。", seeHomeSetup: "查看設定方法",
  personalAppearance: "APP 圖示", customAppIcon: "自訂 APP 圖示", customAppIconHint: "圖示只儲存在這台裝置，不會上傳到伺服器。",
  iconClassic: "經典", iconMonochrome: "黑白極簡", iconPink: "粉色極簡", iconSystem: "系統質感", iconCartoon: "卡通小眼睛", iconCute3d: "3D 可愛", iconNone: "不要圖示",
...
Object.assign(messages.en, {
  ...
  productPositionText: "Daily weight fluctuates, and short-term changes may be small enough to make you wonder whether keeping a record is worthwhile.\nEven so, I hope this tool helps turn weighing in into a small ritual. Keeping a steady record can remind you to stay aware of your health and make your own adjustments to sleep, food, and movement.",
  syncTipTitle: "Cloud sync is recommended", syncTipText: "These records are backed up under a local user ID, but clearing browser data can still make them inaccessible. Turn on sync to recover them with a passcode across devices.", goEnableSync: "Turn it on", doNotRemind: "Don't remind me",
  ...
  personalAppearance: "App icon", customAppIcon: "Custom app icon", customAppIconHint: "The icon stays on this device and is never uploaded to the server.",
...
Object.assign(messages.ja, {
  ...
  productPositionText: "体重は毎日変動し、短期間では大きく変わらないため、記録に価値があるのか疑問に思うこともあるかもしれません。\nそれでも、このツールが体重記録を小さな儀式のような習慣に変える手助けになればと願っています。記録を続けることで健康状態への意識を保ち、睡眠、食事、運動を自分で調整するきっかけになります。",
  syncTipTitle: "クラウド同期がおすすめです", syncTipText: "記録はローカルユーザーIDでバックアップされますが、ブラウザーデータを消去すると開けなくなる場合があります。同期をオンにするとパスコードで複数端末から復元できます。", goEnableSync: "オンにする", doNotRemind: "今後表示しない",
  homeTipTitle: "ホーム画面に体重カレンダーを追加", homeTipText: "ホーム画面に追加すると、APPのようにすぐ開けます。先に好きなアイコンも選べます。", seeHomeSetup: "設定方法を見る",
  personalAppearance: "APPアイコン", customAppIcon: "APPアイコンを変更", customAppIconHint: "アイコンはこの端末だけに保存され、サーバーへ送信されません。",
...
Object.assign(messages.ko, {
  ...
  productPositionText: "체중은 매일 변하고 단기간에는 변화가 크지 않아, 이런 기록이 정말 가치가 있는지 의문이 들 수 있습니다.\n그래도 이 도구가 체중을 기록하는 일을 작은 의식과 같은 습관으로 바꾸어 주길 바랍니다. 꾸준한 기록은 건강 상태를 계속 의식하고, 수면과 식사, 운동을 스스로 조절하도록 일깨워 줍니다.",
  syncTipTitle: "클라우드 동기화를 권장합니다", syncTipText: "기록은 로컬 사용자 ID로 백업되지만 브라우저 데이터를 삭제하면 열 수 없을 수 있습니다. 동기화를 켜면 암호로 여러 기기에서 복구할 수 있습니다.", goEnableSync: "켜기", doNotRemind: "다시 알리지 않기",
  homeTipTitle: "홈 화면에 체중 달력 추가", homeTipText: "홈 화면에 추가하면 APP처럼 더 빠르게 열 수 있고, 먼저 원하는 아이콘도 고를 수 있습니다.", seeHomeSetup: "설정 방법 보기",
  personalAppearance: "APP 아이콘", customAppIcon: "APP 아이콘 설정", customAppIconHint: "아이콘은 이 기기에만 저장되며 서버로 업로드되지 않습니다.",
